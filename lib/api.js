...
export async function getWritings(amount = 100, tag) {
	let entries

	if (tag === 'polish' || tag === 'english') {
		const isEnglish = tag === 'english' ? true : false

		entries = await fetchGraphQL(
			`query {
        writingCollection(where: { english: ${isEnglish} }, order: date_DESC, limit: ${amount}) {
          items {
		...
		entries = await fetchGraphQL(
			`query {
        writingCollection(where: { tags_contains_some: "${tag}" }, order: date_DESC, limit: ${amount}) {
          items {
            ${WRITINGS_LIST_GRAPHQL_FIELDS}
          }
        }
...
export async function getWritingsAmount(tag) {
	let entries
	...
	if (tag === 'polish' || tag === 'english') {
		...
		entries = await fetchGraphQL(
			`query {
        writingCollection(where: { english: ${isEnglish} }, limit: 100) {
          items {
		...
		entries = await fetchGraphQL(
			`query {
        writingCollection(where: { tags_contains_some: "${tag}" }, limit: 100) {
          items {
            ${WRITINGS_LIST_AMOUNT}
          }
        }
